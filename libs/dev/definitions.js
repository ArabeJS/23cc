...
class Definitions {
    ...
    constructor(options) {
        ...
        ['client', 'server', 'component'].forEach(function(folder) {
            fs.readdir('./node_modules/coc-messages/' + folder, (err, files) => {
                if (err) {
                    console.log('error opening node-modules/coc-messages/' + folder + ': ' + err);
                    process.exit(1);
                }

                files.forEach(file => {
                    var json = JSON.parse(fs.readFileSync('./node_modules/coc-messages/' + folder + '/' + file, 'utf8'));
